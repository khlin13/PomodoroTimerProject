...
void checkBtnClick() {
  ...
  boolean timesUp = (curTime-lastReleasedTime[0]>3000) && 
                    (curTime-lastReleasedTime[1]>3000) && 
                    (curTime-lastReleasedTime[2]>3000) &&
                    (curTime-lastClickedTime[0]>3000) && 
                    (curTime-lastClickedTime[1]>3000) && 
                    (curTime-lastClickedTime[2]>3000);
  
  if (timesUp) {
    isTimeSet = true;
    selected = false;
    if (focusIntv > 0 && restMin > 0) {
      isRestIntvSet = true;
    }
    else {
      isRestIntvSet = false;
...
void increaseDigit() {
  if (timerMode) {
    if (selected && timerHr < 23) {
      timerHr += 1;
    }
    else if (!selected && timerMin < 59) {
      timerMin += 1;
    }
  }
  else {
    if (selected && restMin < 99) {
      restMin += 1;
    }
    else if (!selected && focusIntv < 9) {
      focusIntv += 1;
    }
  }
}
...
void decreaseDigit() {
  if (timerMode) {
    if (selected && timerHr > 0) {
      timerHr -= 1;
    }
    else if (!selected && timerMin > 0) {
      timerMin -= 1;
    }
  }
  else {
    if (selected && restMin > 0) {
      restMin -= 1;
    }
    else if (!selected && focusIntv > 1) {
      focusIntv -= 1;
    }
  }
}
...
void startTimer() {
  ...
  focusMillis = timerHr*3600000+timerMin*60000;
  restMillis = restMin*60000;
...
void tickTimer() {
  if (millis() >= expTime) {
    ...
    if (isResting){
      ...
      newTimerPercent = (expTime-millis())/millisIn1per + 1;
    }
